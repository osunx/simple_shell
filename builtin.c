...
/**
* execute_alias - Execute the built-in alias command.
*
* @input: The input command.
*/
...
/* Check if any arguments are provided after "alias" */
if (strlen(input) > 6)
{
/* Extract name and value */
name = stringtok(input + 6, "=");
value = stringtok(NULL, "\n");
if (value == NULL)
{
/* Print alias for the given name */
alias = get_environment(name);
if (alias != NULL)
{
printf("%s='%s'\n", name, alias);
}
...
/* Define or overwrite alias if needed */
if (overwrite)
{
/* 9 for "alias='' " */
alias = malloc(stringlen(name) + stringlen(value) + 9);
if (alias == NULL)
{
perror("malloc");
return;
...
/* Use the set_environment function to set the alias */
set_environment(name, value, 1, 0);

free(alias); /* Free the dynamically allocated memory */
}
}
